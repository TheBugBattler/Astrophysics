# -*- coding: utf-8 -*-
"""
Created on Wed May 22 23:34:54 2024
...
datos_discos_jairo = pd.read_csv('magnitudes_absolutas_discos_jairo.txt', header=None, delim_whitespace=True, names=['ra', 'dec', 'mag_abs'])
datos_discos_pablo = pd.read_csv('magnitudes_absolutas_discos_pablo.txt', header=None, delim_whitespace=True, names=['ra', 'dec', 'mag_abs'])
...
# Unir los datos de galaxias con discos sin eliminar duplicados
datos_discos = pd.concat([datos_discos_jairo, datos_discos_pablo])

# Fusionar con el DataFrame de todas las galaxias para etiquetar las galaxias
datos_fusionados = datos_discos.merge(datos_clasificaciones, on=['ra', 'dec', 'mag_abs'], how='left')
...
labels_finos = [f"{b+0.25:.2f} to {b:.2f}" for b in bins_finos[:-1]]
...
# Calcular el punto medio de la fracción de barras por bin
resumen_bins_finos['punto_medio'] = resumen_bins_finos['fraccion_segura'] + 0.5 * (resumen_bins_finos['fraccion_superior'] - resumen_bins_finos['fraccion_segura'])
...
plt.plot(resumen_bins_finos.index, resumen_bins_finos['punto_medio'], color='red', marker='o', linestyle='-', linewidth=2, label='Punto medio de barras')
